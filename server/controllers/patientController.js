...
export const getPatients = async (req, res) => {
  try {
    ...
    if (search && typeof search === 'string' && search.trim().length > 0) {
      ...
      // Fetch ALL patients with status filter only (no search in DB query)
      const statusQuery = {};
      if (status !== 'All') {
      ...
      const filteredPatients = allPatients.filter(patient => {
        const patientData = {
          ...
          contactNo: patient.contactNo?.toLowerCase() || '',
...
export const searchPatients = async (req, res) => {
  try {
    ...
    const patients = await Patient.searchByName(searchTerm)
      .select('name email contactNo age patientId status')
      .limit(limit);
